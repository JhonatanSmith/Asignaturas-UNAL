...
#ENTRADA DE DATOS 
ni=rep(4,5) #No. réplicas por nivel
...
#AJUSTE MODELO DE ANÁLISIS DE VARIANZA 
...
library(lsmeans)
lsmeans(modelo2,~nitrato) #esta función hace lo mismo que mismediastratamientos()
...
#INTERVALOS DE TUKEY
TukeyHSD(modelo2, conf.level = 0.95) #Función es cargada por defecto por librería stats 
...
duncan.test(modelo2,"nitrato",alpha=0.05,group=TRUE,console=TRUE) #Rango múltiple de Duncan
LSD.test(modelo2,"nitrato",group=TRUE,console=TRUE) #Método LSD

#librería multcomp. Para Dunnett asume que primer nivel es el control
