...
## Funci?n R que representa datos bivariados, junto con elipses de confianza
...
representa2=function(x,alfa1,alfa2,texto){ # Los datos se encuentran en la matriz x
  ...
  teta=seq(0,2*pi,length=101)   ## Vector con los ?ngulos
...
##################### 
## Funci?n R que representa datos bivariados, junto con el vector de medias
...
##################### 
## obtenci?n del coeficiente de asimetr?a muestral
...
##############################
## Funci?n para Gr?fico de perfiles para cada variable 
...
HT2_CU=function(x,C,mu_0,alfa){
  mu=as.vector(apply(x,2,mean));s=var(x)  
  n=nrow(x);p=ncol(x)         
  k<-nrow(C)        ## n?mero de contrastes
  df1=k             
  df2=n-k           
  T2<-n*t(C%*%mu-mu_0)%*%solve(C%*%s%*%t(C))%*%(C%*%mu-mu_0)
  c<-( (n-1)*k )/(n-k)
  F0<-(1/c)*T2                    
  F_tabla=qf(1-alfa,df1,df2)
  valor_p=1-pf(F0,df1,df2)
  resultados=list(Media=mu,T2=T2,c=c,F0=F0,df1=df1,df2=df2,
                  F_Tabla=F_tabla,Valor_p=valor_p) 
  format(resultados, digits = 6)
}
...
sigma_sigma0_ngrande=function(x,Sigma_0,alfa){
  ...
  result=list(Lamda_est=lamda_est,df=df,
              Chi_Tabla=chi_tabla,Valor_P=valor_p) 
