...
Credit<-read.csv(file="Credit.csv", stringsAsFactors=TRUE,header=T,sep=',',dec='.')
Credit1=Credit[,3:13]
...
scatter3D(x, y, z, pch = 19, cex = 1,col=c('red'),
          ...
          surf = list(x = x.pred, y = y.pred, z = z.pred,col='#006633',
                      facets = NA, fit = fitpoints), main = "Advertising Dataset ISLR with interaction")
...
fit.lm2<-lm(mpg~poly(horsepower,2),data=Auto)
fit.lm5<-lm(mpg~poly(horsepower,5),data=Auto)
list(degree1=summary(fit.lm)$r.squared,degree2=summary(fit.lm2)$r.squared,degree5=summary(fit.lm5)$r.squared)
...
fit.loess2<-loess(residu2~fitted2,span=1.1)
...
Auto=ISLR::Auto
dim(Auto)
fit.lm_n<-lm(mpg~horsepower,data=Auto)
#Sample size=2n
Auto2=rbind(Auto,Auto)
dim(Auto2)
fit.lm_2n<-lm(mpg~horsepower,data=Auto2)
summary(fit.lm_n)$coefficients
summary(fit.lm_2n)$coefficients
length_n_1=confint(fit.lm_n)[1,2]-confint(fit.lm_n)[1,1]
length_n_2=confint(fit.lm_n)[2,2]-confint(fit.lm_n)[2,1]
length_2n_1=confint(fit.lm_2n)[1,2]-confint(fit.lm_2n)[1,1]
length_2n_2=confint(fit.lm_2n)[2,2]-confint(fit.lm_2n)[2,1]
list(IC_n=confint(fit.lm_n),IC_2n=confint(fit.lm_2n))
c(length_n_1,length_2n_1,length_n_2,length_2n_2)
...
x = rnorm(N, 0, 9)
par(mfrow=c(1,2))
y_hetero = b0 + b1*x + rnorm(N, mean=0, sd=sigma*sqrt(exp(g1 + g2*x)))
mod.hetero = lm(y_hetero~x)
mod.homo   = lm(log(y_hetero)~x)
fitted1=mod.hetero$fitted.values
#fitted2=mod.homo$fitted.values
res1=mod.hetero$residuals
#res2=mod.homo$residuals
loess1=loess(res1~fitted1,span=80)
f1<-loess1$fitted
d1<-seq(-1,2.5,length.out=length(f1))
pred.int<-predict(mod.hetero,data.frame(f1=d1),interval="prediction")
mydata <- data.frame(cbind(res1,pred.int))
p <- ggplot(mydata, aes(fit, res1)) +
  geom_point() +
  stat_smooth(method = lm,color="red")
# 3. Add prediction intervals
p + geom_line(aes(y = upr), color = "blue", linetype = "dashed")+
  geom_line(aes(y = -upr), color = "blue", linetype = "dashed")

fitted2=mod.homo$fitted.values
res2=mod.homo$residuals
loess2=loess(res2~fitted2,span=0.8)
newdata=data.frame(res2,fitted2)
pred.int2<-loess.sd(newdata,nsigma=1.96,span=1,degree=1)
plot(fitted2,res2,pch=1,col="grey",ylim=c(-5,1.8),xlim=c(-1.5,1.5))
lines(pred.int2$x,pred.int2$y,col="red",lwd=2)
lines(pred.int2$x,pred.int2$upper,col="blue",lwd=2)
lines(pred.int2$x,pred.int2$lower,col="blue",lwd=2)
###
...
lm.fit=lm(y~x)
d<-seq(-2,2,length.out = N)
outlier=data.frame(x=0.3,y=7)
newdata=data.frame(y,x)
newdata1=rbind(newdata,outlier)
lm.fit1=lm(y~x,newdata1)
d<-seq(-2,2,length.out = N)
par(mfrow=c(1,3))
plot(x,y,pch=1,cex=1.1,ylab="Y",xlab="X",ylim = c(-4,7.3))
points(x=0.3,y=7,col="orange",cex=1.3,pch=19)
#Without the outlier
lines(d,predict(lm.fit,data.frame(fitted.value=d)),col="blue",lwd=2)
#Including the outlier
lines(d,predict(lm.fit1,data.frame(fitted.value=d)),col="red",lwd=2)
rse_whout<-summary(lm.fit)$sigma
rse_whith<-summary(lm.fit1)$sigma
c(RSE_SIN=rse_whout,RSE_CON=rse_whith)
resid<-lm.fit1$residuals
fitted<-lm.fit1$fitted.values
studres<-studres(lm.fit1)
plot(fitted,resid,pch=1,cex=1.1,ylim=c(-1,5),ylab="Residuals",xlab="Fitted values")
points(x=2.577,y=4.3933,col="orange",cex=1.35,pch=19)
abline(h=0,col="purple")
plot(fitted,studres,pch=1,cex=1.1,ylim=c(-1,5),ylab="Studentized Residuals",xlab="Fitted values")
points(x=2.577,y=4.3628,col="orange",cex=1.35,pch=19)
abline(h=0,col="purple")
...
lines(d,predict(lm.fit1,data.frame(fitted.value=d)),col="red",lwd=2)
legend("bottomright",legend=c("Without Leverage","With Leverage"),col=c("blue","red"),lty=c(2,1),cex=0.6)

dataEllipse(x, y, levels=c(0.975),ylim=c(-7,15),xlim=c(-4,6),lty=2,grid=FALSE,center.pch=1,center.cex=0,xlab="X",ylab="Y",col=c("black","limegreen"),lwd=1)
points(x=c(0.3,3.5),y=c(7,14),col=c("orange","purple"),cex=1.3,pch=19)
...
plot(leverage,studres,pch=1,cex=1.1,ylim=c(-1,5),xlim=c(0,0.25),ylab="Studentized Residuals",xlab="Leverage")
points(x=c( 0.02796440,0.06926729),y=c(3.52165422,3.84071451),col=c("orange","purple"),cex=1.3,pch=19)
abline(h=0,col="purple")
