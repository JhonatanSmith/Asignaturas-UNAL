...
```
Cuando se aplica la funcion as.factor, el orden de los mismos cambia. Entonces simplemente, se cambiará los niveles de la variable como factor a un valor numerico, siendo 1 enero, 2 febrero y asi sucesivamente. Luego, se transforma en string (Caracter) para obtener finalmente lo siguiente en la estructura del dataframe

```{r}
df %>% head(1)
```
Ahora mes, es un numero. 
...
Se procede a graficas la serie de tiempo de los cerdos

```{r}
df = df[order(df$date),] # ordenando por fecha mas vieja

plot(df$date, df$sacrificio_totales, type = 'l',main = 'Sacrificios totales',xlab = 'Fecha',ylab = 'Total sacrificios ')
```
...
View(cerditos_2_la_venganza)

df = cerditos_2_la_venganza
df %>% head(3)
```
